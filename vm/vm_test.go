...
func TestHashLiterals(t *testing.T) {
	tests := []vmTestCase{
		...
		},
		{
			"{1: 2, 2: 3}",
			map[object.HashKey]int64{
				(&object.Integer{1}).HashKey(): 2,
				(&object.Integer{2}).HashKey(): 3,
			},
		},
		{
			"{1 + 1: 2 * 2, 3 + 3: 4 * 4}",
			map[object.HashKey]int64{
				(&object.Integer{2}).HashKey(): 4,
				(&object.Integer{6}).HashKey(): 16,
			},
		},
	}
...
func TestBuiltinFunctions(t *testing.T) {
	tests := []vmTestCase{
		...
		{`len("hello world")`, 11},
		{
			`len(1)`,
			&object.Error{
				Message: "arguments to `len` not supported, got INTEGER",
			},
		},
		{
			`len("one", "two")`,
